...
// loop through instructions, move crates from source to dest, maintaining order
for (let step of instructions) {
  const [amount, source, destination] = step
    .slice(5)
    .split(/move | from | to /);

  // use splice to cut a portion of the source out and push it into destination
  crateStacks[destination].push(
    ...crateStacks[source].splice(crateStacks[source].length - amount)
  );
}
