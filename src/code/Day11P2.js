...
// init product of all monkey's divisibility tests (testNums)
// we % the worry by this because it's divisible by all the monkey testNums, so the result will satisfy the monkeys the same as if it were the massive number it would otherwise be
let monkeyProd = 1;
...
for (let round = 0; round < 10000; round++) {
  for (let monkey of monkeys) {
    ...
    while (monkey.items.length) {
      ...
      monkey.items[0] = Math.floor(monkey.items[0] % monkeyProd);
